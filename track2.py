...
cv2.createTrackbar('topleftx', 'Tracking',96,320,nothing)
cv2.createTrackbar('toplefty', 'Tracking',73,240,nothing)
cv2.createTrackbar('toprightx', 'Tracking',267,320,nothing)
cv2.createTrackbar('toprighty', 'Tracking',73,240,nothing)

cv2.createTrackbar('bottomleftx', 'camera',70,320,nothing)
cv2.createTrackbar('bottomlefty', 'camera',184,240,nothing)
cv2.createTrackbar('bottomrightx', 'camera',289,320,nothing)
cv2.createTrackbar('bottomrighty', 'camera',184,240,nothing)
...
while(1):
    ...
    circles = cv2.findContours(closing,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
    ...
    if circles is not None:
        for i in circles[0,:]:
            ...
            wherex = (int(round(i[0])) + wherex) / 2
            wherey = (int(round(i[1])) + wherey) / 2
            dia = (int(round(i[2])) + dia) / 2
            if True:#int(round(i[2])) < 30:
                cv2.circle(frame,(wherex,wherey),dia,(0,255,0),5)
                cv2.circle(frame,(wherex,wherey),2,(0,255,0),10)
            # else draw it in red
            ...
            if time.time() - tick > 1:
                ...
                if ((dx * dx) > 400) or ((dy * dy) > 400): 
                    ...
                    publish.single("cycy42/where/diff", payload = diff,hostname="win8.local", qos=0,retain=True) 
                    if abs(diff) < 360:
                        publish.single("cycy42/where/direction", payload = direction,hostname="win8.local", qos=0,retain=True)   
