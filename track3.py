...
while(1):
    ...
    if len(circles) > 0:
        ...
        wherex = int(x)#(int(round(i[0])) + wherex) / 2
        wherey = int(y)#(int(round(i[1])) + wherey) / 2
        dia = int(radius)#(int(round(i[2])) + dia) / 2
        ...
        if time.time() - tick > 1:
            ...
            dy = wherey - oldy
            if ((dx * dx) + (dy * dy))> 100: 
                publish.single("where/x", payload = (wherex-160),hostname="127.0.0.1", qos=0,retain=True)
                publish.single("where/y", payload = (120 - wherey),hostname="127.0.0.1", qos=0,retain=True)
